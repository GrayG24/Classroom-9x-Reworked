...
export const Sidebar = ({ 
  ...
  const rawMenuItems = [
    ...
    { id: AppRoute.SUMMER, label: 'Summer', icon: Sun, isReleased: true, accentColor: 'text-orange-400' },
  ...
  return (
    <motion.div 
      ...
      animate={{ 
        ...
        x: 20,
        y: isExpanded ? 30 : 65,
        borderRadius: isExpanded ? "2.4rem" : "2.8rem",
        opacity: 1
      }}
      transition={{ 
        type: "spring", 
      ...
      className="fixed left-0 top-0 bg-black/40 backdrop-blur-[40px] border border-white/5 z-50 flex flex-col shadow-[20px_0_100px_rgba(0,0,0,0.6)]"
    >
      {/* Logo Section */}
      <div className="p-6 flex flex-col items-center w-full shrink-0">
        <motion.div 
          layout
          className={`flex items-center gap-4 w-full h-12 ${isExpanded ? 'justify-start px-2' : 'justify-center'}`}
        >
          <motion.div 
            layout
        ...
        {menuItems.map((item) => {
          ...
          return (
            <motion.button
              ...
              className={`h-14 flex items-center relative overflow-hidden ${
                isActive 
              ...
              <AnimatePresence mode="popLayout" initial={false}>
                  {isExpanded && (
                    <motion.span 
                      ...
                      className={`ml-4 text-[10px] font-black uppercase tracking-[0.2em] italic relative z-10 whitespace-nowrap pointer-events-none ${isActive ? (isRed ? 'text-white' : 'text-black') : ''}`}
                    >
                      {item.label}
                    </motion.span>
                  )}
      ...
      <div className={`p-4 mt-auto border-t border-white/10 flex flex-col gap-4 ${isExpanded ? 'items-start' : 'items-center'} w-full shrink-0`}>
        ...
        >
          ...
          {isExpanded && (
            <div className="ml-auto opacity-20 group-hover:opacity-100 group-hover:translate-x-1 transition-all">
        ...
        <div className={`flex flex-col gap-2 w-full ${isExpanded ? 'px-1' : 'items-center'}`}>
          <div className={`flex items-center transition-all bg-white/[0.02] border border-white/5 ${isExpanded ? 'gap-3 px-3 py-2.5 rounded-xl w-full' : 'w-10 flex-col justify-center h-auto min-h-12 py-2 px-0 rounded-xl'}`}>
            <AnimatePresence initial={false}>
              {isExpanded ? (
                <motion.div 
                  key="expanded-stats"
